    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="detailForm" name="detailForm" class="form-horizontal" autocomplete="off">
                    <div class="row">
                        <div class="col-sm-12">
                            <input type="hidden" name="id" id="id">
                            <input type="hidden" name="action" id="action" value="Add">
                            {{-- <div class="form-group">
                                <label for="purchase_id" class="col-sm-12 control-label">Pembelian</label>
                                <div class="col-sm-12">
                                    <select name="purchase_id" id="purchase_id" class="form-control" required>
                                        ...
                                        @foreach ($purchases as $purchase)
                                            <option value="{{ $purchase->id }}">{{ $purchase->nomor_pembelian }} - {{ $purchase->account_detail->nama_rincian_akun }} - {{ $purchase->keterangan }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div> --}}
